...
def proceed(keywords=None, file_dest="data",
            resources_dest="resource", browser_info=None, config_dest="water_config.json"):
    if keywords is None:
        keywords = []
    ...
    if "selected" in config:
        ...
        for key in response.keys():
            if key in selected[0]:
                if len(selected[0]) > 1:
                    ...
                    return_info['_id'] = response['data'][0]['_id']
                    return_info['createdAt'] = response['data'][0]['createdAt']
                    return_info['water_vapor'] = response['data'][0]['water_vapor']
        ...
        if (config_dest == "water_config.json"):
            generate_img("_id", "createdAt", "water_vapor")
