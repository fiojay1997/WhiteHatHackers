...
def proceed(keywords=None, file_dest="data",
            resources_dest="resource", browser_info=None, config_dest="air_config.json"):
    if keywords is None:
        keywords = []
        ...
        for key in response.keys():
            if key in selected[0]:
                if len(selected[0]) > 1:
                    ...
                    return_info['AQI'] = response['stations'][0]['AQI']
                    return_info['PM10'] = response['stations'][0]['PM10']
                    return_info['PM25'] = response['stations'][0]['PM25']
